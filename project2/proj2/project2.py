    ...
    for ents_per_sent in tqdm(sent2ents):
        ...
        for relation_type in required_relation_types:
            ...
            for idx in relation_indexs:
                ...
                tokens = [token.text for token in sent] #word_tokenize(sent.text)
...
def process_relations(model, pairs, sents, confidence_threshold, required_relations_spanbert, required_relations, X, X_with_scores):
    if isinstance(model, SpanBERT):
        return extraction_spanbert(model, pairs, confidence_threshold, required_relations_spanbert, X, X_with_scores)
    else:
        return extraction_gpt3(model, sents, required_relations, X)


def extraction_spanbert(spanbert_model, relations, t, required_relation, required_tuples, required_tuples_with_scores):
    ...
    for sent_idx, relations_per_sent in enumerate(tqdm(relations)):
        ...
        for relation, relation_pred in zip(relations_per_sent, relations_pred_per_sent):
            ...
            if relation_type == required_relation and confidence_score > t:
...
def extraction_gpt3(gpt3_model, sents, required_relation, X):
    ...
    for sent in tqdm(sents):
        prompt_candidates = {
                "Schools_Attended" : "Given a sentence input, extract all triplets that satisfy the following relation: \
                        ...
                        Example output: 'Jeff Bezos'-'Schools_Attended'-'Princeton University' \
                        Input: {}".format(sent.text),
                "Work_For": "Given a sentence input, extract all triplets that satisfy the following relation: \
                        Required relation and the required output format:  'person'-'Work_For'-'organization' \
                        Example output: 'Alec Radford'-'Work_For'-'OpenAI'\
                        Input: {}".format(sent.text),
                "Live_In": "Given a sentence input, extract all triplets that satisfy the following relation: \
                        Required relation and the required output format:  'person'-'Live_In'-'location, city, state, province or country' \
                        Example output: 'Mariah Carey'-'Live_In'-'New York City'\
                        Input: {}".format(sent.text),
                "Top_Member_Employees": "Given a sentence input, extract all triplets that satisfy the following relation: \
                        Required relation and the required output format:  'organization'-'Top_Member_Employees'-'person' \
                        Example output: 'Nvidia'-'Top_Member_Employees'-'Jensen Huang'\
                        Input: {}".format(sent.text)
                }
        response = gpt3_model(
            model="text-davinci-003",
            prompt=prompt_candidates[required_relation],
            max_tokens=200,
            temperature=0,
            top_p=1,
            frequency_penalty=0,
        ...
        relation_pairs = response_text.strip().split(",")
        for relation_pair in relation_pairs:
            relation_pair = tuple(relation_pair.replace("'", "").split("-"))
            relation_preds.append(relation_pair)
        time.sleep(1.5)
    return relation_preds, None
...
def main():
    ...
    required_relations = r2relation_types_printed[r]
    ...
    while True:
        ...
        for result_idx, result in enumerate(search_results):
            ...
            if curr_url not in used_urls:
                ...
                if webpage_text != None:
                    ...
                    X, X_with_scores = process_relations(model, pairs_required, sents_required, t, required_spanbert_relations, required_relations, X, X_with_scores)
                else:
        ...
        if extraction_method == "-spanbert":
            ...
            if len(X) > k:
                ...
                with open("transcript{}.txt".format(extraction_method), "a") as output_file:
                    output_file.write("================== ALL RELATIONS for {} ( {} ) =================\n".format(r2relation_types_printed[r], k))
